...
namespace Valloon.Trading.Backtest
{
    static class Sol5
    {
        public static void Run()
        {
            ...
            //{
            //    DateTime startTime = new DateTime(2022, 2, 24, 0, 0, 0, DateTimeKind.Utc);
...

            {
                ...
                //Test2(2, 75.8f, 83f, startTime); return;
                //{
                //    Logger logger = new Logger($"{DateTime.Now:yyyy-MM-dd  HH.mm.ss}  -  buy = 1");
                //    for (float i = 28f; i < 35; i += .5f)
                //    {
                //        float result = Test2(1, 27.2f, i, startTime);
        ...
        static float Test2(int buyOrSell, float minRSI, float maxRSI, DateTime startTime, DateTime? endTime = null)
        {
            ...
            for (float minDiff = 0; minDiff < .22f; minDiff += .05f)
            //float minDiff = 0;
            {
                for (float maxDiff = 2.75f; maxDiff <= 13.5f; maxDiff += .5f)
                //float maxDiff = 7.95f;
                {
                    logger.WriteLine($"\n\n----    minDiff = {minDiff}    maxDiff = {maxDiff}    ----\n");
                    for (float closeX = 0.0030f; closeX <= 0.0090f; closeX += 0.0005f)
                    //float closeX = 0.007f;
                    {
                        for (float stopX = 0.01f; stopX <= closeX * 5; stopX += 0.005f)
                        //float stopX = 0.035f;
                        {
                            int succeedCount = 0, failedCount = 0;
                            float totalProfit = 0;
        ...
        static float TestBuyOrSell4(DateTime startTime, DateTime? endTime = null)
        {
            ...
            {
                string paramText = @"2	83	100	0	15	0.003	0.014	2
2	75.8	83	0	13.5	0.0073	0.036	0.777
2	72.4	75.8	0.022	7.95	0.007	0.036	0.777
2	69	72.4	0.033	4.05	0.0085	0.038	0.737
2	67.2	69	0.4	3.35	0.006	0.03	0.933
1	0	16	0	15	0.008	0.03	0.933
1	16	17.6	0	12.2	0.0075	0.032	0.875
1	17.6	19.9	0.81	4.6	0.008	0.032	0.875
1	19.9	22.7	0.81	2.75	0.006	0.024	1.167
1	22.7	27.2	1.52	2.35	0.0036	0.014	2
";
