...
namespace Valloon.Trading.Backtest
{
    public class LunaDao
    {
        ...
        public static List<LunaBin> SelectAll(string binSize)
        {
            using (SQLiteCommand command = Connection.CreateCommand())
            {
                command.CommandText = $"SELECT * FROM sol_{binSize} ORDER BY timestamp";
                SQLiteDataReader dr = command.ExecuteReader();
                List<LunaBin> list = new List<LunaBin>();
                while (dr.Read())
                {
                    try
                    {
                        LunaBin m = new LunaBin
                        {
        ...
        public static int Insert(LunaBin m, string binSize)
        {
