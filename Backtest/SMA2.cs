...
namespace Valloon.BitMEX.Backtest
{
    ...
    static class SMA2
    {
        ...
        static float Benchmark()
        {
            ...
            for (int smaLength1 = 24; smaLength1 <= 576; smaLength1 += 24)
            //int smaLength1 = 144;
            {
                ...
                for (int smaLength2 = 12; smaLength2 <= smaLength1 / 2; smaLength2 += 12)
                //int smaLength2 = 72;
                {
                    ...
                    for (float limitX = 0.0025f; limitX <= 0.05f; limitX += .0025f)
                    //float limitX = .02f;
                    {
                        for (float closeX = 0.0025f; closeX <= 0.05f; closeX += .0025f)
                        //float closeX = .02f;
                        {
                            for (float stopX = 0.01f; stopX <= 0.025f; stopX += .0025f)
                            //float StopX = .012f;
                            {
                                int tryCount = 0;
