...
namespace Valloon.BitMEX.Backtest
{
    ...
    static class Candle
    {
        static string SYMBOL = BitMEXApiHelper.SYMBOL_SOLUSD;
        ...
        public static void Run()
        {
            ...
            //Loader.Load(SYMBOL, "1m", new DateTime(2022, 5, 20, 0, 0, 0, DateTimeKind.Utc)); return;

            //{
            //    DateTime startTime = new DateTime(2022, 5, 18, 0, 0, 0, DateTimeKind.Utc);
        ...
        static float Benchmark()
        {
            ...
            Logger logger = new Logger($"{DateTime.Now:yyyy-MM-dd  HH.mm.ss}    BenchmarkSAR    bin = {binSize1}    fee = {makerFee} - {takerFee}    {startTime:yyyy-MM-dd} ~ {endTime:yyyy-MM-dd} ({totalDays:N0}) days");
            logger.WriteLine("\n" + logger.LogFilename + "\n");
                                        ...
                                        //float max2 = 0.017f;
                                        {
                                            ...
                                            for (int i = 1; i < count - 1; i++)
                                            {
                                                ...
                                                else if (position == 1)
                                                {
                                                    ...
                                                    else if (closeLimit > 0 && (float)list1[i].High / positionEntryPrice > 1 + closeLimit)
                                                    {
                                                        tryCount++;
                                                ...
                                                else if (position == -1)
                                                {
                                                    ...
                                                    else if (closeLimit > 0 && (float)positionEntryPrice / list1[i].Low > 1 + closeLimit)
                                                    {
                                                        tryCount++;
                                            ...
                                            //    position = 0;
                                            //}
                                            totalProfit /= CandleQuote.GetX(SYMBOL);
        ...
        static string Test()
        {
            ...
            string result = "";
            result += Test(new DateTime(2022, 5, 1, 0, 0, 0, DateTimeKind.Utc)) + "\r\n\r\n";
        ...
        static string Test(DateTime startTime, DateTime? endTime = null)
        {
            ...
            double[] rsiArray = RSI.CalculateRSIValues(binList.ToArray(), 14);
            ...
            {
                if (list[i].RSI < 30 && list[i - 2].RSI > list[i - 1].RSI && list[i - 1].RSI < list[i].RSI)
                {
                    tryCount++;
                    int positionEntryPrice = list[i].Close;
                    for (int j = i + 1; j < count; j++)
                    {
                        if (list[j].Low < positionEntryPrice - stopLoss)
                        {
                            lossCount++;
                            logger.WriteLine($"{tryCount} \t {list[i].Timestamp:yyyy-MM-dd HH:mm:ss} \t {list[i].Open} / {list[i].High} / {list[i].Low} / {list[i].Close} \t <Failed>", ConsoleColor.Red);
                            break;
                        }
                        else if (list[j].High > positionEntryPrice + closeLimit)
                        {
                            profitCount++;
                            logger.WriteLine($"{tryCount} \t {list[i].Timestamp:yyyy-MM-dd HH:mm:ss} \t {list[i].Open} / {list[i].High} / {list[i].Low} / {list[i].Close} \t <Succeed>", ConsoleColor.Green);
                            break;
                        }
                    }
                }
                //if (position == 0)
                //{
                //    if (list[i].Timestamp.Hour >= 0 && list[i].Timestamp.Hour <= 3) continue;
                //    if (list[i].Timestamp.DayOfWeek == DayOfWeek.Saturday) continue;
                ...
                //        //else
                //        logger.WriteLine($"{list[i].Timestamp:yyyy-MM-dd HH:mm:ss} \t {list[i].Open} / {list[i].High} / {list[i].Low} / {list[i].Close} \t {pSar1.Sar:F0} / {pSar1.IsReversal} \t <SHORT>");
                //    }
                ...
                //        //else
                //        logger.WriteLine($"{list[i].Timestamp:yyyy-MM-dd HH:mm:ss} \t {list[i].Open} / {list[i].High} / {list[i].Low} / {list[i].Close} \t {pSar1.Sar:F0} / {pSar1.IsReversal} \t <LONG>");
                //    }
                //}
                //else if (position == 1)
                //{
                //    if ((float)list[i].Low / positionEntryPrice < 1 - stopLoss || pSar1.IsReversal.Value && pSar1.OriginalSar.Value < pSar1.Sar.Value)
                //    {
                //        tryCount++;
                //        int close = (int)Math.Ceiling(positionEntryPrice * (1 - stopLoss));
                //        if (pSar1.IsReversal.Value) close = (int)Math.Max(close, pSar1.OriginalSar.Value);
                ...
                //        if (maxHeight < (float)profit / positionEntryPrice) maxHeight = (float)profit / positionEntryPrice;
                //        if (profit > 0)
                //            logger.WriteLine($"{list[i].Timestamp:yyyy-MM-dd HH:mm:ss} \t {list[i].Open} / {list[i].High} / {list[i].Low} / {list[i].Close} \t {pSar1.Sar:F0} / {pSar1.IsReversal} \t entry = {positionEntryPrice} \t profit = {profit} \t total = {totalProfit} \t % = {finalPercent:F4} / {finalPercent2:F4}");
                //        else
                //            logger.WriteLine($"{list[i].Timestamp:yyyy-MM-dd HH:mm:ss} \t {list[i].Open} / {list[i].High} / {list[i].Low} / {list[i].Close} \t {pSar1.Sar:F0} / {pSar1.IsReversal} \t entry = {positionEntryPrice} \t profit = {profit} \t total = {totalProfit} \t % = {finalPercent:F4} / {finalPercent2:F4}", ConsoleColor.Red);
                //        //position = -1;
                //        //positionEntryPrice = close;
                //        position = 0;
                //        if (pSar1.IsReversal.Value)
                ...
                //        }
                //    }
                //    else if ((float)list[i].High / positionEntryPrice > 1 + closeLimit)
                //    {
                //        tryCount++;
                //        int close = (int)Math.Floor(positionEntryPrice * (1 + closeLimit));
                //        int profit = close - positionEntryPrice;
                ...
                //    else if (pSar1.IsReversal.Value && pSar1.OriginalSar.Value >= pSar1.Sar.Value)
                //    {
                //        logger.WriteLine($"{list[i].Timestamp:yyyy-MM-dd HH:mm:ss} \t {list[i].Open} / {list[i].High} / {list[i].Low} / {list[i].Close} \t {pSar1.Sar} / {pSar1.IsReversal} / {pSar1.OriginalSar:F0} \t \t ERROR", ConsoleColor.Red);
                //    }
                //}
                //else if (position == -1)
                //{
                //    if ((float)positionEntryPrice / list[i].High < 1 - stopLoss || pSar1.IsReversal.Value && pSar1.OriginalSar.Value > pSar1.Sar.Value)
                //    {
                //        tryCount++;
                //        int close = (int)Math.Floor(positionEntryPrice / (1 - stopLoss));
                //        if (pSar1.IsReversal.Value) close = (int)Math.Min(close, pSar1.OriginalSar.Value);
                ...
                //        }
                //    }
                //    else if ((float)positionEntryPrice / list[i].Low > 1 + closeLimit)
                //    {
                //        tryCount++;
                //        int close = (int)Math.Ceiling(positionEntryPrice / (1 + closeLimit));
                //        int profit = positionEntryPrice - close;
            ...
            float avgProfit = totalProfit / tryCount;
            string result = $"{startTime} ~ {endTime} ({totalDays} days) \t close = {closeLimit:F4} \t stop = {stopLoss:F4}" +
                $"\r\ncount = {count} \t try = {tryCount} / {profitCount} / {lossCount} : {closeCount} / {stopCount} / {csrCount} \t h = {minHeight:F4} / {maxHeight:F4} \t p = {totalProfit:F2} \t avg = {avgProfit:F2} \t % = {finalPercent:F4} / {finalPercent2:F4} / {finalPercent3:F4} / {finalPercent5:F4} / {finalPercent10:F4}";
            logger.WriteLine($"\r\n{result}");
            return result;
        }
