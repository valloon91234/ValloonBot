...
namespace Valloon.BitMEX.Backtest
{
    static class EMACross
    {
        ...
        static float Benchmark()
        {
            ...
            var emaShortList = quoteList.GetEma(9).ToList();
            var emaLongList = quoteList.GetEma(26).ToList();
            emaShortList.RemoveAll(x => x.Date < startTime || endTime != null && x.Date > endTime.Value);
            emaLongList.RemoveAll(x => x.Date < startTime || endTime != null && x.Date > endTime.Value);

            //for (int i = 0; i < count; i++)
            //{
            //    Console.WriteLine($"{list[i].Timestamp:yyyy-MM-dd HH:mm:ss} \t {list[i].Open} / {list[i].High} / {list[i].Low} / {list[i].Close} \t {emaShortList[i].Ema:F4}    {emaLongList[i].Ema:F4}");
            //}
            ...
            List<Dictionary<string, float>> topList = new List<Dictionary<string, float>>();
            {
                ...
                int rsiValue = 100;
                {
                    ...
                    //float closeX = 0;
                    {
                        for (float stopX = 0; stopX < 0.1f; stopX += 0.005f)
                        //float stopX = 0;
                        {
                            ...
                            float maxProfit = 0, maxLoss = 1, finalPercent = 1, finalPercent2 = 1;
                            int position = 0;
                            ...
                            for (int i = 2; i < count - 1; i++)
                            {
                                if (position == 0)
                                {
                                    if (buyOrSell == 1)
                                    {
                                        if (emaShortList[i - 1].Ema.Value <= emaLongList[i - 1].Ema.Value && emaShortList[i].Ema.Value > emaLongList[i].Ema.Value)
                                        {
                                ...
                                else if (position == 1)
                                {
                                    ...
                                    else if (emaShortList[i].Ema.Value < emaLongList[i].Ema.Value)
                                    {
                                        tryCount++;
                                        flipCount++;
                            ...
                            if (finalPercent > 1f && succeedCount > 0)
                            {
                                ...
                                logger.WriteLine($"rsi = {rsiLength} / {rsiValue} \t limit = {closeX:F4} / {stopX:F4} \t count = {tryCount} / {skipCount} / {succeedCount} / {failedCount} / {successRate:F2} \t % = {finalPercent:F4} / {finalPercent2:F4}");
                            }
                            else if (succeedCount > 0)
                            {
                                logger.WriteLine($"rsi = {rsiLength} / {rsiValue} \t limit = {closeX:F4} / {stopX:F4} \t count = {tryCount} / {skipCount} / {succeedCount} / {failedCount} / {successRate:F2} \t % = {finalPercent:F4} / {finalPercent2:F4}", ConsoleColor.DarkGray, false);
                            }
                        }
                    }
                }
