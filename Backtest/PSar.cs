                                    ...
                                    //float start2 = step2;
                                    {
                                        ...
                                        //float max2 = 0.017f;
                                        {
                                            ...
                                            for (int i = 1; i < count - 1; i++)
                                            {
                                                ...
                                                if (position == 0)
                                                {
                                                    if (pSar1.IsReversal.Value)
                                                    {
                                                        ...
                                                        if (pSar1.OriginalSar.Value < pSar1.Sar.Value)
                                                        {
                                                            position = -1;
                                                            positionEntryPrice = (int)pSar1.OriginalSar.Value;
                                                            positionStopPrice = list1[i].Low * (1 + stopProfit);
                                                        }
                                                        //else if (pSar1.OriginalSar.Value > pSar1.Sar.Value && pSar2.Sar.Value <= pSar1.Sar.Value)
                                                        else if (pSar1.OriginalSar.Value > pSar1.Sar.Value)
                                                        {
                                                            position = 1;
                                                            positionEntryPrice = (int)pSar1.OriginalSar.Value;
                                                            positionStopPrice = list1[i].High * (1 - stopProfit);
                                                        }
                                                    }
                                                    //else if (pSar2.IsReversal.Value && pSar1.Sar > list[i].Open && pSar2.Sar >= pSar1.Sar)
                                                    //{
                                                ...
                                                {
                                                    positionStopPrice = Math.Max(positionStopPrice, list1[i].High * (1 - stopProfit));
                                                    if ((float)list1[i].Low / positionEntryPrice < 1 - stopLoss || list1[i].Low < positionStopPrice || pSar1.IsReversal.Value && pSar1.OriginalSar.Value < pSar1.Sar.Value)
                                                    {
                                                        tryCount++;
                                                        int close = (int)Math.Ceiling(Math.Max(positionStopPrice, positionEntryPrice * (1 - stopLoss)));
                                                        if (pSar1.IsReversal.Value) close = (int)Math.Max(close, pSar1.OriginalSar.Value);
                                                        int profit = close - positionEntryPrice;
                                                        totalProfit += profit - positionEntryPrice * takerFee;
                                                        finalPercent *= (float)close / positionEntryPrice - takerFee;
                                                ...
                                                {
                                                    positionStopPrice = Math.Min(positionStopPrice, list1[i].Low * (1 + stopProfit));
                                                    if ((float)positionEntryPrice / list1[i].High < 1 - stopLoss || list1[i].High > positionStopPrice || pSar1.IsReversal.Value && pSar1.OriginalSar.Value > pSar1.Sar.Value)
                                                    {
                                                        tryCount++;
                                                        int close = (int)Math.Floor(Math.Min(positionStopPrice, positionEntryPrice / (1 - stopLoss)));
                                                        if (pSar1.IsReversal.Value) close = (int)Math.Min(close, pSar1.OriginalSar.Value);
                                                        int profit = positionEntryPrice - close;
                                                        totalProfit += profit - positionEntryPrice * takerFee;
                                                        finalPercent *= (float)positionEntryPrice / close - takerFee;
                                            ...
                                            if (totalProfit > 0 && finalPercent > 1f && tryCount >= totalDays / 2)
                                            {
                                                ...
                                                logger.WriteLine($"{start1:F6} / {step1:F6} / {max1:F6} \t limit = {closeLimit:F4} / {stopLoss:F4} / {stopProfit:F4} \t count = {tryCount} / {succeedCount} / {failedCount} / {successRate:F4} \t h = {minHeight:F4} / {maxHeight:F4} \t p = {totalProfit:F2} \t avg = {avgProfit:F2} \t % = {finalPercent:F4} / +{score:F4}");
                                            }
                                            else
                                            {
