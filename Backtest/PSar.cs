...
namespace Valloon.BitMEX.Backtest
{
    static class PSar
    {
        static string SYMBOL = BitMEXApiHelper.SYMBOL_XBTUSD;
        ...
        static float Benchmark()
        {
            ...
            for (float stopLoss = 0.01f; stopLoss <= 0.02f; stopLoss += .001f)
            //float stopLoss = .012f;
            {
                for (float closeLimit = stopLoss; closeLimit <= 0.05f; closeLimit += .001f)
                //float closeLimit = .02f;
                {
                    for (float step1 = 0.001f; step1 <= 0.02f; step1 += .0005f)
                    //float step1 = .0015f;
                    {
                        ...
                        //float start1 = .00186f;
                        {
                            for (float max1 = .01f; max1 <= 0.2; max1 += .005f)
                            //float max1 = .03f;
                            {
