...
namespace Valloon.BitMEX
{
    class BitMEXApiHelper
    {
        ...
        public List<Order> GetActiveOrdersAll()
        {
            Dictionary<string, string> param = new Dictionary<string, string>
            {
                ["symbol"] = SYMBOL,
            ...
            CreateSignature("GET", "/order", BuildQueryData(param));
            List<Order> list = OrderApiInstance.OrderGetOrders(SYMBOL, null, null, null, null, true);
            List<Order> resultList = new List<Order>();
            foreach (Order order in list)
            {
                if ((order.OrdStatus == "New" || order.OrdStatus == "PartiallyFilled"))
        ...
        public Order OrderNewLimit(string side, int qty, int price)
        {
            Dictionary<string, string> param = new Dictionary<string, string>
            {
                ["symbol"] = SYMBOL,
            ...
            return OrderApiInstance.OrderNew(SYMBOL, side, null, qty, price, null, null, null, null, null, null, "Limit"); ;
        }

        public List<Order> OrderNewLimitBulk(List<Order> orderList)
        {
            int count = orderList.Count;
            for (int i = 0; i < count; i++)
            {
                orderList[i].Symbol = SYMBOL;
                orderList[i].OrdType = "Limit";
            }
            JObject jObject = new JObject()
            {
        ...
        public Order OrderNewLimitClose(string side, int qty, int price)
        {
            Dictionary<string, string> param = new Dictionary<string, string>
            {
                ["symbol"] = SYMBOL,
            ...
            CreateSignature("POST", "/order", null, BuildQueryData(param));
            return OrderApiInstance.OrderNew(SYMBOL, side, null, qty, price, null, null, null, null, null, null, "Limit", null, "Close"); ;
        }

        public Order OrderNewMarket(string side, int qty)
        {
            Dictionary<string, string> param = new Dictionary<string, string>
            {
                ["symbol"] = SYMBOL,
            ...
            CreateSignature("POST", "/order", null, BuildQueryData(param));
            return OrderApiInstance.OrderNew(SYMBOL, side, null, qty, null, null, null, null, null, null, null, "Market");
        }

        public Order OrderNewStopMarket(string side, int qty, int stopPx)
        {
            Dictionary<string, string> param = new Dictionary<string, string>
            {
                ["symbol"] = SYMBOL,
            ...
            CreateSignature("POST", "/order", null, BuildQueryData(param));
            return OrderApiInstance.OrderNew(SYMBOL, side, null, qty, null, null, stopPx, null, null, null, null, "Stop");
        }

        public Order OrderNewLimitClose(string side, int price)
        {
            Dictionary<string, string> param = new Dictionary<string, string>
            {
                ["symbol"] = SYMBOL,
            ...
            CreateSignature("POST", "/order", null, BuildQueryData(param));
            return OrderApiInstance.OrderNew(SYMBOL, side, null, null, price, null, null, null, null, null, null, "Limit", null, "Close"); ;
        }

        public Order OrderNewStopMarketClose(string side, int stopPx)
        {
            Dictionary<string, string> param = new Dictionary<string, string>
            {
                ["symbol"] = SYMBOL,
            ...
            CreateSignature("POST", "/order", null, BuildQueryData(param));
            return OrderApiInstance.OrderNew(SYMBOL, side, null, null, null, null, stopPx, null, null, null, null, "Stop", null, "Close");
        }

        public Order OrderNewTakeProfitMarketClose(string side, int stopPx)
        {
            Dictionary<string, string> param = new Dictionary<string, string>
            {
                ["symbol"] = SYMBOL,
            ...
            CreateSignature("POST", "/order", null, BuildQueryData(param));
            return OrderApiInstance.OrderNew(SYMBOL, side, null, null, null, null, stopPx, null, null, null, null, "MarketIfTouched", null, "Close");
        }

        public Position GetPosition()
        {
            CreateSignature("GET", "/position");
            List<Position> list = PositionApiInstance.PositionGet();
